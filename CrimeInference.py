...
class CrimeInference:
    ...
    def __init__(self):
        self.weapons = ["Poignard", "Revolver", "Chandelier",
                        "Corde", "Cle_Anglaise", "Matraque"]
        self.rooms = ["Cuisine", "Salle_de_bain", "Salon",
                      "Cave", "Bureau", "Bibliotheque", "Hall"]
        self.persons = ["Rose", "Violet", "Pervenche",
                        "Olive", "Moutarde", "Leblanc"]
    ...
    def base_clauses(self):
        ...
        self.body_mark_clause = 'MarquePoignard({})'
        self.body_mark_clause = 'MarqueRevolver({})'
        self.body_mark_clause = 'MarqueCle_Anglaise({})'
        self.body_mark_clause = 'MarqueMatraque({})'
        self.body_mark_clause = 'MarqueChandelier({})'
        ...
        for i in range(len(self.rooms)):
            for j in range(len(self.rooms)):
                if i != j:
                    ...
                    self.clauses.append(
                        expr(self.room_different_clause.format(self.rooms[i], self.rooms[j])))
        ...
        for i in range(len(self.weapons)):
            for j in range(len(self.weapons)):
                if i != j:
                    ...
                    self.clauses.append(
                        expr(self.weapon_different_clause.format(self.weapons[i], self.weapons[j])))
    ...
    def inference_rules(self):
        ...
        self.clauses.append(
            expr('EstMort(x) & Personne_Piece(x, y) ==> PieceCrime(y)'))

        # Determiner l'arme du crime
        self.clauses.append(
            expr('PieceCrime(x) & Arme(y) & Piece_Arme(y, x) ==> ArmeCrime(y)'))
        self.clauses.append(
            expr("EstMort(x) & MarqueCou(x) ==> ArmeCrime(Corde)"))
        self.clauses.append(
            expr("EstMort(x) & MarquePoignard(x) ==> ArmeCrime(Poignard)"))
        self.clauses.append(
            expr("EstMort(x) & MarqueRevolver(x) ==> ArmeCrime(Revolver)"))
        self.clauses.append(
            expr("EstMort(x) & MarqueCle_Anglaise(x) ==> ArmeCrime(Cle_Anglaise)"))
        self.clauses.append(
            expr("EstMort(x) & MarqueMatraque(x) ==> ArmeCrime(Matraque)"))
        self.clauses.append(
            expr("EstMort(x) & MarqueChandelier(x) ==> ArmeCrime(Chandelier)"))
    ...
    def get_crime_hour(self):
        ...
        if not result:
            ...
            return result[x]
    # Demander à la base de connaissances l'heure après le crime(1h de plus)
